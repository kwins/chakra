...
void chakra::cmds::CommandClusterMeet::execute(char *req, size_t len, void* data) {
    ...
    if (err) {
        fillError(meetMessageResponse.mutable_error(), 1, err.what());
    } else if (meet.ip().empty() || meet.port() <= 0) {
        fillError(meetMessageResponse.mutable_error(), 1, "ip or port empty");
    } else {
        DLOG(INFO) << "[cluster] meet message request: " << meet.DebugString();
        auto peer = clsptr->getPeer(meet.ip(), meet.port());
        if (peer) {
            fillError(meetMessageResponse.mutable_error(), 1, "peer has exist in cluster");
        } else {
