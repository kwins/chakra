...
void chakra::cmds::CommandReplicaSyncResponse::execute(char *req, size_t len, void *data,
    ...
    if (err) {
        LOG(ERROR) << "Replicate response deserialize error " << err.what();
    } else if (syncMessageResponse.error().errcode() != 0 || syncMessageResponse.db_name().empty()) {
        LOG(ERROR) << "Replicate response error: " << syncMessageResponse.DebugString();
    } else {
        link->setLastInteractionMs(utils::Basic::getNowMillSec());
        auto replicateDB = link->getReplicateDB(syncMessageResponse.db_name());
        if (!replicateDB) {
            LOG(ERROR) << "Replicate response db " << syncMessageResponse.db_name() << " not found in server.";
            return;
        }
        ...
        {
            LOG(INFO) << "Replicate accepted a FULL db(" << syncMessageResponse.db_name() << ") sync FROM " << link->getPeerName();
        ...
        {
            LOG(INFO) << "Replicate accepted a PART db(" << syncMessageResponse.db_name() << ") sync FROM " << link->getPeerName();
        ...
        default:
        {
            LOG(INFO) << "Replicate accepted a BAD sync message " <<  syncMessageResponse.DebugString();
            break;
        }
        }
    }
